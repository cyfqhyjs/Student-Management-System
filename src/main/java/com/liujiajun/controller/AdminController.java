...
import com.liujiajun.config.BcryptUtil;
...
public class AdminController {
    ...
    public String addStudentUI(Model model) throws Exception {
     ...
     public String addStudent(StudentCustom studentCustom, Model model) throws Exception {
         ...
         // 对密码进行加密处理
         String encryptedPassword = BcryptUtil.encode("123"); // 使用BCrypt加密密码
    ...
    public String userPasswordRest(Userlogin userlogin) throws Exception {
        ...
        if (u != null) {
            ...
            // 对密码进行加密处理
            String encryptedPassword = BcryptUtil.encode(userlogin.getPassword());
    ...
    // 显示发布通知的表单页面
    @GetMapping("/showNoticeForm")
    public String showNoticeForm() {
        return "adminNoticeForm"; // 返回表单页面
    }

    // 处理发布通知的 POST 请求
    @PostMapping("/adminNotice")
    public String adminNotice(@RequestParam String notice) {
        int rowsAffected = noticeMapper.adminNotice(notice);
        if (rowsAffected > 0) {
            return "redirect:/admin/showNotice"; // 跳转到通知列表页面
        } else {
            return "error"; // 返回错误页面
        }
    }
    ...
    public String showNotice(Model model) {
        List<Notice> notices = noticeMapper.showNotice();
        model.addAttribute("notices", notices);
        return "showNotice"; // 返回通知列表页面
    }
